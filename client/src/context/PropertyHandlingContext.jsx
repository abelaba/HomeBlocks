...
import { getEthereumContract, getNextMonthDate } from '../utils/helperFunctions'
...
export const PropertyHandlingProvider = ({ children }) => {
  ...
  const addProperty = async (
    ...
    try {
      await axios.post(`${rentingURL}/add`, formData, config)
  ...
  const viewMyProperties = async () => {
    try {
      const response = await axios.get(`${rentingURL}/viewMyProperties`, config)
      if (response.status == 200) {
        return response.data
      } else {
        toast('Error')
  ...
  const acceptTenant = async (tenantAccount, propertyId) => {
    ...
    try {
      const propertyContract = await getEthereumContract()
      await propertyContract.addTenant(
        propertyId,
        tenantAccount,
        time.format('X')
      )
      ...
      toast(
        'Tenant added successfully, it may take sometime to add changes to the blockchain'
      )
    } catch (error) {
      toast.dismiss(toastId)
      console.log(error)
  ...
  const removeTenant = async (propertyId) => {
    ...
    try {
      ...
      toast.dismiss(toastId)
      toast(
        'Tenant removed successfully, it may take sometime to add changes to the blockchain'
      )
      return property
    } catch (error) {
      console.log(error.response)
  ...
  const makePayment = async (property, chatId) => {
    try {
      ...
      getNextMonthDate(new Date(property.paymentDate * 1000))
