...
export const PropertyHandlingProvider = ({ children }) => {
    ...
    const addProperty = async (name, description, address, image, price, ownerShip) => {
        const hashDigest = sha256(ownerShip);
        const propertyOwnershipHash = Base64.stringify(hmacSHA512(hashDigest, connectedAccount));
        console.log(propertyOwnershipHash);
        if (!token) {
    ...
    const acceptTenant = async (tenantAccount, propertyId) => {
        console.log({ tenantAccount, propertyId });
        const time = getNextMonthDate(new Date().getTime());
        console.log(time.format("X"));

        try {
            const propertyContract = await getEthereumContract();
            const property = await propertyContract.addTenant(propertyId, tenantAccount, time.format("X"));
            console.log(property);
            return property;
        } catch (error) {
            console.log(error);
    ...
    const sendTransaction = async (chatId, message) => {
        ...
        try {
            const response = await axios.put(`http://localhost:${PORT}/api/chat/sendMessage`, { chatId: chatId, message: message, messageType: TRANSACTION }, { headers: { "auth-token": token } });
    ...
    return (
        <PropertyHandlingContext.Provider value={{
            addProperty,
            viewMyProperties,
            viewProperty,
            viewAllProperties,
